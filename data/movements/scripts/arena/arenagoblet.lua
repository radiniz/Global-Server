function onStepIn(cid, item, position, fromPosition) 
	...
	if item.actionid == 42360 then 
		if getPlayerStorageValue(cid, 42360) ~= 1 then 
			...
			doSetItemActionId(goblet, "description", "It is given to the courageous victor of the barbarian arena greenhorn difficulty.\nAwarded to " .. getCreatureName(cid) .. ".") 
	...
	elseif item.actionid == 42370 then 
		if getPlayerStorageValue(cid, 42370) ~= 1 then 
			...
			doSetItemActionId(goblet, "description", "It is given to the courageous victor of the barbarian arena scrapper difficulty.\nAwarded to " .. getCreatureName(cid) .. ".") 
			doTileAddItemEx({x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}, goblet) 
		end 
	elseif item.actionid == 42380 then 
		if getPlayerStorageValue(cid, 42380) ~= 1 then 
			...
			doSetItemActionId(goblet, "description", "It is given to the courageous victor of the barbarian arena warlord difficulty.\nAwarded to " .. getCreatureName(cid) .. ".") 
			doTileAddItemEx({x=gobletPos.x,y=gobletPos.y-1,z=gobletPos.z}, goblet) 
		end 
	end 
